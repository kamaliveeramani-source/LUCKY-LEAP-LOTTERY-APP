...
export default function GameShell({ game, config, accent, rules, children, selectedValue, selectedLabel, selectedOption, submitValue, submitLabel = "Place bet", resultLabel = "Result", historyTabs = ["Result history", "Analyze", "My orders"] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (token) {
      ...
      const nextBalance = Number(walletResponse.data?.wallet);
  ...
  const submit = async () => {
    if (busy) return;
    if (!token) { setError("Sign in to place a bet."); return; }
    if (!currentRound) { setError("No active round"); return; }
    if ((submitValue === null || submitValue === undefined || submitValue === "") || !Number.isFinite(amount) || amount <= 0) { setError("Choose a selection and a valid bet amount."); return; }
    if (balance === null) { setError("Wallet is unavailable. Please refresh and try again."); return; }
    if (total > balance) { setError("Insufficient wallet balance. Please add cash to continue."); return; }
    setBusy(true); setError(""); setNotice("");
    try {
      const response = await API.post("/ticket/buy", { gameId: game.id, roundId: currentRound.id, selectedValue: String(submitValue), GameOptionId: selectedOption?.id, stakeAmount: amount, multiplier });
      const updatedBalance = Number(response.data?.wallet);
      if (Number.isFinite(updatedBalance)) setBalance(updatedBalance);
      setNotice("Bet placed successfully");
      await refresh();
    } catch (requestError) {
      setError(requestError.response?.data?.message || requestError.message || "Unable to place the bet.");
    } finally { setBusy(false); }
  };
  return <main className={`game-experience ${accent}`}>
    <header className="game-experience-header"><button type="button" className="game-back-button" onClick={() => navigate(-1)} aria-label="Go back">‹</button><div><span className="game-eyebrow">{game.category || "Game"}</span><h1>{game.name}</h1></div><span className="virtual-credit-indicator">{balance === null ? "Loading..." : `₹${balance.toLocaleString("en-IN")}`}</span></header>
