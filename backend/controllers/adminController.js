...
function lotteryPayload(body, partial = false) {
  ...
  for (const [input, field] of Object.entries(mappings)) {
    if (body[input] !== undefined) fields[field] = body[input];
  }

  if (fields.drawTime && fields.drawDate) {
    const drawDate = new Date(fields.drawDate);
    const [hours, minutes] = String(fields.drawTime).split(":").map(Number);
    if (!Number.isNaN(drawDate.getTime()) && Number.isInteger(hours) && Number.isInteger(minutes)) {
  ...
  if (!partial && (!fields.lotteryName || !fields.ticketPrice || !fields.firstPrize || !fields.drawDate)) {
    return { error: "lotteryName, ticketPrice, firstPrize and drawDate are required" };
  }
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        totalWinningAmount: sum(winners, "winningAmount"),
        totalDeposits: sum(wallets, "totalDeposit"),
        totalWithdrawals: sum(wallets, "totalWithdraw"),
      },
    });
  } catch (error) {
...
exports.listLotteries = async (req, res) => {
  try { return res.json({ success: true, data: await Lottery.findAll({ order: [["id", "ASC"]] }) }); }
  catch (error) { return res.status(500).json({ success: false, message: error.message }); }
};

exports.createLottery = async (req, res) => {
  try {
    const { fields, error } = lotteryPayload(req.body || {});
...
exports.getLottery = async (req, res) => {
  try {
    const lottery = await Lottery.findByPk(numberOrNull(req.params.id));
...
exports.updateLottery = async (req, res) => {
  try {
    const lottery = await Lottery.findByPk(numberOrNull(req.params.id));
    if (!lottery) return res.status(404).json({ success: false, message: "Lottery not found" });
    const { fields, error } = lotteryPayload(req.body || {}, true);
    if (error) return res.status(400).json({ success: false, message: error });
    if (fields.lotteryName) {
      const duplicate = await Lottery.findOne({ where: { lotteryName: { [Op.iLike]: fields.lotteryName }, id: { [Op.ne]: lottery.id } } });
      if (duplicate) return res.status(409).json({ success: false, message: "Lottery already exists" });
    }
...
exports.listTickets = async (req, res) => {
  try {
    const where = {};
    const lotteryId = numberOrNull(req.query.lotteryId);
    const userId = numberOrNull(req.query.userId);
...
exports.getTicket = async (req, res) => {
  try {
    const ticket = await Ticket.findByPk(numberOrNull(req.params.id), { include: [{ model: User, attributes: publicUserAttributes }, { model: Lottery }] });
...
exports.getUser = async (req, res) => {
  try {
    const user = await User.findByPk(numberOrNull(req.params.id), { attributes: publicUserAttributes, include: [{ model: Wallet, attributes: { exclude: ["UserId", "createdAt", "updatedAt"] } }, { model: Ticket, include: [Lottery] }] });
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    return res.json({ success: true, data: { user, ticketHistory: user.Tickets || [], bettingHistory: user.Tickets || [] } });
  } catch (error) { return res.status(500).json({ success: false, message: error.message }); }
};

exports.listWinners = async (req, res) => {
  try { return res.json({ success: true, data: await Ticket.findAll({ where: { status: "WON" }, include: [{ model: User, attributes: publicUserAttributes }, { model: Lottery }], order: [["updatedAt", "DESC"]] }) }); }
  catch (error) { return res.status(500).json({ success: false, message: error.message }); }
};

exports.listTransactions = async (req, res) => {
  try {
    const wallets = await Wallet.findAll({ include: [{ model: User, attributes: publicUserAttributes }] });
    const transactions = wallets.flatMap((wallet) => {
      const entries = [["DEPOSIT", wallet.totalDeposit], ["WITHDRAWAL", wallet.totalWithdraw], ["WINNING", wallet.totalWinning]];
      return entries.filter(([, amount]) => Number(amount) > 0).map(([type, amount]) => ({ type, amount: Number(amount), status: "COMPLETED", date: wallet.updatedAt, User: wallet.User }));
    });
    return res.json({ success: true, data: transactions });
  } catch (error) { return res.status(500).json({ success: false, message: error.message }); }
};

exports.drawLottery = (req, res) => drawWinner(req, res);