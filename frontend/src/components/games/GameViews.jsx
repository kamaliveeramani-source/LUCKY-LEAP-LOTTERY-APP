...
export function PokerGame({ game, config }) {
  ...
  return <GameShell game={game} config={config} accent="game-poker" rules="Choose a poker outcome for the current issue and place a wallet bet." selectedValue={selected?.value ?? selected?.label} selectedLabel="Poker prediction" selectedOption={selected} submitValue={selected?.value ?? selected?.label}>
...
export function RummyGame({ game, config }) {
  ...
  return <GameShell game={game} config={config} accent="game-rummy" rules="Select a configured rummy pattern for the open round and place a wallet bet." selectedValue={selected?.value ?? selected?.label} selectedLabel="Rummy pattern" selectedOption={selected} submitValue={selected?.value ?? selected?.label}>
...
export function CricketGame({ game, config }) {
  const categories = asOptions(config.categories, ["INTERNATIONAL", "T20", "IPL"]); const events = asOptions(config.events || config.matches, game.options?.length ? game.options : ["MATCH WINNER", "TOP SCORE", "TOTAL RUNS"]); const [category, setCategory] = useState(categories[0]); const [selected, setSelected] = useState(null);
    return <GameShell game={game} config={config} accent="game-cricket" rules="Choose a configured cricket category and event prediction for the current issue. Match outcomes and results come from the backend." selectedValue={selected?.value ?? selected?.label} selectedLabel={`${category?.label || category?.value} event`} selectedOption={selected} submitValue={selected?.value ?? selected?.label} resultLabel="Cricket event">
  <div className="cricket-scoreboard"><span>LIVE</span><strong>INTERNATIONAL • {category?.label || category?.value}</strong><small>Next event selection</small></div><div className="cricket-category-grid">{categories.map((item) => <button type="button" key={item.id || item.value || item.label} className={category === item ? "selected" : ""} onClick={() => setCategory(item)}>{item.label}</button>)}</div><div className="cricket-event-grid">{events.map((item) => <button type="button" key={item.id || item.value || item.label} className={selected === item ? "selected" : ""} onClick={() => setSelected(item)}><span>🏏</span>{item.label}</button>)}</div>
  </GameShell>;
}
