...
export default function Bets() {
  ...
  const [selection, setSelection] = useState(() => location.state || (() => { try { return JSON.parse(sessionStorage.getItem("betSelection")) || null; } catch { return null; } })());
  ...
  useEffect(() => { if (location.state) { setSelection(location.state); sessionStorage.setItem("betSelection", JSON.stringify(location.state)); } }, [location.state]);
  ...
  const refreshData = useCallback(async () => {
    ...
    const [roundsResponse, walletResponse, betsResponse] = await Promise.all([API.get(`/games/${selection.gameId}/rounds`), API.get("/wallet/balance"), API.get("/ticket/mytickets")]);
    const now = Date.now();
    ...
    setWalletBalance(Number(walletResponse.data?.wallet || 0)); setMyBets((betsResponse.data?.tickets || []).filter((ticket) => Number(ticket.GameDefinitionId) === Number(selection.gameId)));
  }, [selection?.gameId, token]);
  useEffect(() => { refreshData().catch((requestError) => setError(requestError.response?.data?.message || "Unable to load game data.")); }, [refreshData]);
